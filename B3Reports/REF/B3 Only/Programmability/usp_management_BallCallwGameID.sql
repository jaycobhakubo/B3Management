...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[usp_management_BallCallwGameID]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[usp_management_BallCallwGameID]
GO
...
CREATE proc [dbo].[usp_management_BallCallwGameID]
    (@session int,
    @GameID int,
	@BallCallType int,
